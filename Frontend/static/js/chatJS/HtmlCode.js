...
const Friends = (friend, type, blockedby) => {
    return `<div class="Ruser w-100 py-2 px-2 mb-2 d-flex align-items-center justify-content-between ${blockedby.some(block=>block.id === friend.id) && 'modeBlockUser'}">
...
const message = (msg, target_user_id) => {
    return `<div class="slide-in-blurred-top-1 CcontentMessage d-flex align-items-end ml-2 position-relative" >
                <div class="d-flex gap-3">
                   ...
                   <div class="${msg?.user?.id === target_user_id ? 'Cmessage' : 'Cmessage2'} bg-pink ">${escapeHTML(msg?.content)}</div>
                </div>
                <span class="timeChat position-absolute">${msg.user.username} <span class="timeSize fw-normal">${getTime(msg?.created)}</span></span>
            </div>`
}
...
const member = (member) => {
    return `<div class="chUser d-flex justify-content-between align-items-center p-2">
                <div class="d-flex align-items-center gap-1"> 
                    ...
                    <div class="hiddenData d-flex flex-column ">
...
const message_channel =(member, user)=>{
    return `<div class="slide-in-blurred-top-1 CcontentMessage d-flex align-items-end ml-2 position-relative" >
                <div class="d-flex gap-3">
                   ...
                   <div class="Cmessage bg-pink ">${escapeHTML(member?.cmContent)}</div>
                </div>
                <span class="timeChat position-absolute">${user?.username}<span class="timeSize fw-normal">${getTime(member?.cmCreated)}</span></span>
            </div>`
}